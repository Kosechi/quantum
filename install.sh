...
echo 'alias qtm="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/Kosechi/quantum.git" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/Kosechi/quantum.git'"
...
rm -rf "$HOME/.config/@qtm"
rm -rf "$HOME/.config/@quantum"
rm -rf "$HOME/.config/qtm-core"
